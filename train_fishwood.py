...
def utility_fishwood(values):
    values = torch.min(values[:,0], values[:,1]//2)
...
if __name__ == '__main__':
    ...
    gamma = args.gamma; nS = 2; nA = 2
    acc = args.acc
    env = gym.make('FishWood-v0')
    env = OneHotEnv(env, nS)
    if acc:
        env = AccruedWrapper(env)
    # env = TerminalEnv(env , utility)
    actor = Actor(nS+acc*2, nA)
    utility_function = utility_fishwood
    # utility_function = utility

    logdir = f'runs/fishwood/{args.algo}/acc_{acc}/gamma_{gamma}/lr_{args.lr}/'
